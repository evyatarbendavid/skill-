...
plus sources): [`seo-aeo-reference.md`](./seo-aeo-reference.md).
...
- `scripts/audit.py` in this skill automates the mechanically-checkable subset
...
> `seo-aeo-reference.md` §4 for the fake numbers circulating online.
...
## What `audit.py` automates

| Automated | Needs a human or Search Console |
|---|---|
| A1, A2, A3, A4, A5, A7, A8 (partial) | A6 (index status — GSC only) |
| B4, B8, B9 + duplicate detection | B1, B2, B3, B5, B6, B7, B10 |
| C1, C2 (property-level), C3, C4, C5, C7 | C6 (visible-content match) |
| D1, D2, D3, D4 (via PSI field data) | D5–D8 (diagnostic judgment) |
| E1 (partial), E2 (partial), E3, E4, E5, E9 | E6, E7, E8, E10 |
| F1, F7 + heuristics for F2, F4 | F3, F5, F6 |

Items the script cannot decide are reported as `N/A` with a reason, or as
`HUMAN_JUDGMENT` — never silently passed.
